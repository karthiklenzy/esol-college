...
if (isset($_POST['btn-inquiry'])) {
	...
	// preffered branch
			if (isset($_POST['checkbox_malabe'])) {
				if ($preffered_branch == "") {
					...
					$email_hr = "kevin@esol.lk";
					$admission_id = "ADM-MLB_";
				}
				else{
					$preffered_branch .= ", Malabe";
					$email_hr = "farshad@esol.lk";
	...
	if ($error_count == 0) {
	  ...
	  if ($getlastrefno) {
		...
		$parent_array = array('p_name' => $parent_name, 'ref_no' => $new_ref_no, 'p_surname' => $parent_surname, 'p_profession' => $parent_profession, 'p_relation' => $parent_relation, 'p_landline' => $parent_landline, 'p_mobile' => $parent_mobile, 'p_address' => $parent_address, 'p_email' => $parent_email, 'p_register' => $parent_register, 'p_request' => $parent_request, 'branch' => $preffered_branch);

		$parent_sql = $db->query("INSERT INTO tbl_admission (parent_name, admission_reference_number, parent_surname, parent_profession, parent_relation, parent_landline, parent_mobile, parent_address, parent_mail, for_register, for_request, branch_name) VALUES (:p_name, :ref_no, :p_surname, :p_profession, :p_relation, :p_landline, :p_mobile, :p_address, :p_email, :p_register, :p_request, :branch)", $parent_array);
...
$include_file = DOC_ROOT.'pages/admission_form_page.php';
